...
const newChapters = rawChapters.map(ch => {
    ...
    if (ch.id === 1) {
        return {
            ...
            goldenQuotes: ch.goldenQuotes.map(q => ({
                quote: { en: q.quote, bn: q.quote === "All improvement starts within." ? "সমস্ত উন্নতি ভেতর থেকে শুরু হয়।" : q.quote === "We can’t be happy and comfortable in the world if we are not the same in our minds." ? "আমরা যদি মানসিকভাবে শান্ত না থাকি, তবে পৃথিবীতে সুখী হতে পারববিধা।" : q.quote === "With Stoic wisdom, we can guard our minds against the things that disrupt our happiness." ? "স্টয়িক দর্শনের সাহায্যে আমরা আমাদের মনকে রক্ষ করতে পারি।" : "একটি শক্তিশালী মন একটি শান্ত মন।" },
    ...
    // Generic transform for the rest of chapters
    return {
        id: ch.id,
